...
```
I made the GEOID variable numeric.
...
nyincarceration
```
I decided to only look at zip codes in New York City to reduce the amount of data I had to analyze.
...
table3 <- left_join(table2, table1 |> 
          ...
          group_by(GEOID) |> 
          summarize(racial_majority = unique(racial_majority)), by = "GEOID")
...
```
In table 1, I used the inner join function to combine the data from both data sets by zip code using the GEOID name. 
In table 2, I created the variables avg_yearly_filings and avg_yearly_imprison_rate so that my eviction data would be for the whole year instead of month to coincide with the yearly imprisonment rate. 
In table 3, I brought back the racial majority variable that showed the demographics of each zip code by using the left join variable. To make sure the racial majority variable showed up on my plot, I mutated it and changed it to a factor. 
In table 4, I removed the zip code 10173 because it was an outlier and was not allowing me to see the pattern in the plot. 
In table 5, I also removed the zip code 10199. I intend to find out why these zip codes are outliers.
...
ggplot(table5, aes(x = avg_yearly_filings,
  ...
  geom_point() +
  labs(title = "Relationship between eviction filings in New York City and the 
       imprisonment rate in 2020",
       x = "Average Eviction Filings in 2020",
       y = "Imprisonment Rate (per 100,000) in 2020")

```
I created a scatter plot with the average filings for a given zip code as the explanatory variable and the imprisonment rate as the outcome variable. After taking out the outliers, the plot showed a cluster of data at the bottom left for white zip codes, showing that white zip codes have lower numbers of eviction filings and a smaller imprisonment rate. Interestingly, the Latinx zip codes are clustered in a positive direction in the middle of the plot, suggesting that the more eviction filings a Latinx zip code has, the higher the imprisonment rate is for that zip code. The zip codes labeled "Other" (which means another racial/ethnic group is the majority, or there is no group with >50% of the population in the zip code) had a high imprisonment rate no matter the average eviction filings. Black zip codes have a similar average of eviction filings as Latinx zip codes but a higher imprisonment rate than Latinx zip codes. 
